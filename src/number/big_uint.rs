...
//        unsafe { copy_nonoverlapping(val.as_ptr() as *const u8, me.number.as_mut_ptr() as *mut u8, size_u); }
            ...
            + PartialEq + PartialOrd
    {
        ...
        else    // if (U::size_in_bytes() <= T::size_in_bytes()) && (module <= rhs)
        {
            ...
            let mself = self.into_u128().modular_sub(mrhs, modu);
            if mself >= mrhs
            {
    ...
    */
    ...
    /// 
    /// # Output
    /// - If `rhs` is not zero, it returns the remainder of the calculation that
    /// `self` is divided by `rhs`, which is `self` % `rhs`, with wrapping
    /// (modular) addition.
    /// 
    /// # Features
    ...
    /// let _num = 0_u32;
    /// // It will panic.
    /// // multiple = a_biguint.next_multiple_of_uint(_num);
    /// ```
    /// 
    /// # Big-endian issue
    /// It is just experimental for Big Endian CPUs. So, you are not encouraged
    ...
    /// - If `size_of::<T>() * N` <= `128`, this method may panic
    /// or its behavior may be undefined though it may not panic.
    /// - If both `self` and `exp` are zero, the result is undefined
    ...
    /// - If both `self` and `rhs` are zero, the result is undefined
    ...
    /// assert_eq!(res.to_string(), "0");
    /// 
    /// let _a_biguint = U256::from_uint(81_u8);
    /// // It will panic.
    /// // let res = _a_biguint.ilog_uint(0_u8);
    /// 
    /// // It will panic.
    /// // let res = _a_biguint.ilog_uint(1_u8);
    /// 
    /// let _a_biguint = U256::zero();
    /// let _base = 6_u8;
    /// // It will panic.
    ...
    /// 
    /// // It will panic.
    /// // a_biguint.ilog_assign_uint(0_u8);
    /// 
    /// // It will panic.
    ...
    /// let _a_biguint = U256::from_uint(81_u8);
    /// // It will panic.
    /// // let res = _a_biguint.unchecked_ilog_uint(0_u8);
    /// 
    /// // It will panic.
    /// // let res = _a_biguint.unchecked_ilog_uint(1_u8);
    /// 
    /// let _a_biguint = U256::zero();
    /// let _base = 6_u8;
    /// // It will panic.
    ...
    /// It is just experimental for Big Endian CPUs. So, you are not encouraged
    /// to use it for Big Endian CPUs for serious purpose. Only use this crate
    /// for Big-endian CPUs with your own full responsibility.    #[inline]
    pub fn unchecked_ilog_uint<U>(&self, base: U) -> Self
    where U: SmallUInt + Copy + Clone + Display + Debug + ToString
            + Add<Output=U> + AddAssign + Sub<Output=U> + SubAssign
            + Mul<Output=U> + MulAssign + Div<Output=U> + DivAssign
    ...
    pub fn ilog2_assign_uint(&mut self)
    {
        ...
        self.set_flag_bit(flag | log2.get_all_flags());
    }
    ...
    pub fn ilog10_assign_uint(&mut self)
    {
        ...
        self.set_flag_bit(flag | log10.get_all_flags());
    }
    ...
    */
    ...
    /// let m = b_biguint.wrapping_add_uint(2_u8); // == 76801874298166903427690031858186486050853753882811946569946433649008
    /// println!("Originally,\tb_biguint = {}", b_biguint);
    ...
    /// let m = c_biguint.wrapping_add_uint(2_u8); // == 76801874298166903427690031858186486050853753882811946569946433649008
    /// println!("Originally,\tc_biguint = {}", c_biguint);
