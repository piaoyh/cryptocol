...
/// This struct is for just creating simple pseudo-random numbers.
/// The basic algorithm is taken from the rand() function of C standard
/// library and tweaked by the author. So, it is light weight.
/// The algoriithm of the rand() function of C standard library is
/// `random number = seed * 1103515245_u64 + 12345_u64` where `random number`
/// is the next random number in random number sequence, and `seed` is
/// the initial random number and the generated `random number` will be fed
/// as `seed` to this formula to get next `random number`. Owing to its
/// addition operation and overflow, this formula produce pseudo-random
/// numbers non-linearly. You can change the constant values 1103515245_u64
/// and 12345_u64 into other numbers by changing the generic constant
/// parameters MULTIPLIER and ADDER if you want to create simple pseudo-random
/// numbers your own way.
/// 
...
/// statistics.
/// 
/// It is designed to be used to plug as `Random_Engine` in the struct
/// `Random_Generic`.
/// 
/// It is __for non-cryptographic purpose__. So, normally it is OK to use this
/// struct as `Random_Engine` embedded in the struct `Random_Generic` for
