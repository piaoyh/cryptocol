...
Random_Engine for SHA1_generic<5, H0, H1, H2, H3, H4, ROUND, K0, K1, K2, K3, RL1, RL5, RL30>
{
    ...
    fn harvest(&mut self, tangling: u64) -> [u64; 8]
    {
        ...
        let a: [u32; 5] = self.get_HashValue_in_array();
        self.tangle(tangling);
        let b: [u32; 5] = self.get_HashValue_in_array();
        self.tangle(tangling);
        let c: [u32; 5] = self.get_HashValue_in_array();
        self.tangle(tangling);
        let d: [u32; 5] = self.get_HashValue_in_array();
        let mut res = [0_u64; 8];
        for i in 0..5
